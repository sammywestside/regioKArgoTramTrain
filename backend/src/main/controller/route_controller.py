...
#calculate route
@router.get("/delivery_route", response_model=Route)
    ...
    try:
        ...
        if ready: 
            ...
            for package in packages:
                ...
                dest_id = package_destination.id
...
def get_route_steps(robot_id: str = Query(...)):
    try:
        robot = robot_repo.get_robot_by_id(robot_id)

        stations = robot.route.stations

        # path ist eine Liste von (station_id, line_name)
        steps = [
            {"station_id": station.id, "line": train_service.get_line(station.id)}
            for station in stations
        ]
        return {"steps": steps, "total_time": robot.route.travel_time}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
