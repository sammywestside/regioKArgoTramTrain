...
from src.main.model.models import Robot, Package, Coordinates
...
class RobotService:
    ...
    def add_package_to_robot(self, package: Package) -> int:
        try:
            if self.isAtCapacity(package.weight):
                ...
                print(
                    f"Package {package.id} added to robot {self.robot.name}.")
    ...
    def remove_one_package_from_robot(self) -> int:
        if self.robot.packages:
            # entfernt das letzte Paket, du kannst hier andere Logik machen
            removed_package = self.robot.packages.pop()
            self.robot.num_packages -= 1
            print(
                f"Package {removed_package.id} removed from robot {self.robot.name}.")
        else:
    ...
    def calculate_changes(self, current_pos: Coordinates):
        try:
            current_station_id = self.train_service.get_station_id_by_coords(
                Coordinates(lat=current_pos.lat, long=current_pos.long))
